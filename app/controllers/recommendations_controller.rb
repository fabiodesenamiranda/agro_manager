...
class RecommendationsController < ApplicationController
  ...
  def destroy
    ...
    respond_to do |format|
      format.html {
 redirect_to(recommendations_path, status: :see_other, notice: "Recommendation was successfully destroyed.") }
  ...
  private
    ...
    def set_recommendation
      @recommendation = Recommendation.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def recommendation_params
      params.expect(recommendation: [ :service_id, :client_id, :suggested_date ])
    end
end
